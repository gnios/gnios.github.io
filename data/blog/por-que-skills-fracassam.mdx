...
draft: false
summary: Skills fracassam porque são criadas fora de contexto. A IA gera genérico quando não tem exemplos, padrões e edge cases específicos do seu domínio. Três pilares para criar skills que as pessoas confiem.
...
A IA, sem esse contexto, gera genérico. E genérico em engenharia de software é quase sinônimo de inútil.
...
**Depois, execute o trabalho passo a passo com a IA.** Não peça "faz tudo". Peça "me ajuda a atualizar esse PR conforme os padrões". Deixa acontecer. A IA vai tentar, você vai corrigir onde errar, a IA vai ajustar. Nesse processo, um padrão começa a emergir — os steps que funcionam, as correções que você fez, os formatos reais de entrada e saída, as coisas específicas do projeto que você teve que explicar.
...
O princípio é simples: mantenha o `SKILL.md` pequeno — abaixo de 500 linhas, abaixo de 5k tokens. Só as instruções core que o agente precisa em toda execução. Conteúdo detalhado vai para arquivos separados em `references/`, `assets/`, `scripts/`, e só é carregado sob demanda.

O que faz a diferença não é ter os arquivos separados — é **dizer ao agente quando carregar cada um**:
